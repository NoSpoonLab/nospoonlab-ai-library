...
namespace Tests.AgentTest.Language
{
    ...
    public class EnglishAgentUnitTest : TestBase
    {
        ...
        public void Setup()
        {
            _agent = new Agent();
            _agent2 = new Agent();
            _agent.Initialize("John Lin", "en","a dedicated pharmacy merchant, prioritizes community well-being in Willow Market", 5, 5, 5, 5, 5);
            _agent2.Initialize("Sam Moore", "en", "a retired engineer with a penchant for woodworking and gardening, values sharing his knowledge", 7, 7, 7, 7, 7);
        }
        ...
        public async Task TestEmbeddingVector()
        {
            ...
            var observationEmbedding = await _agent.GetEmbedding(observation);
            var wordEmbedding = await _agent.GetEmbedding("Animals");
        ...
        public async Task TestMemory()
        {
            ...
            var observationEmbedding = await _agent.GetEmbedding("Do you usually do a lot of sport?");
        ...
        public async Task TestAgentMessage()
        {
            ...
            var response = (await _agent.RequestAgentMessage(question,"Jose")).message;
            Console.WriteLine("Test question: " + question);
            Console.WriteLine("Agent response: " + response);
            question = "Who are you?";
            response = (await _agent.RequestAgentMessage(question, "Jose")).message;
            Console.WriteLine("Test question: " + question);
            Console.WriteLine("Agent response: " + response);
        ...
        public async Task TestAgentMessageOutOfContext()
        {
            ...
            foreach (var it in _inputsThatMustReturnEmpty)
            {
                ...
                var response = (await _agent.RequestAgentMessage(it,"Jose")).message;
                Console.WriteLine("Test question: " + it);
                Console.WriteLine("Agent response: " + response);
                try
                {
                    Assert.IsTrue(response.Contains("Sorry") || response.Contains("don't know") || response.Contains("<|im_end|>"));
                }
                catch (Exception e)
                {
                    Console.WriteLine("Failed on question: " + it);
                    throw;
                }
