...
namespace AICore.Infrastructure.NoSpoonAI
{
    ...
    public class NoSpoonAIClient
    {
        ...
        public void Initialize()
        {
            ...
            if(!_services.Exists<IAIGPTService>())
            {
                var aiGptService = DIContainer.Get<IAIGPTService>();
                _services.Add(aiGptService);
            }
            
            if (_services.Exists<IEmbeddingService>()) return;
            var embeddingService = DIContainer.Get<IEmbeddingService>();
            _services.Add(embeddingService);
        }
        #endregion
