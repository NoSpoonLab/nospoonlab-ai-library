...
{
    public class GameView : MonoBehaviour, IDragHandler, IEndDragHandler, IBeginDragHandler
    {
        ...
        private void InitializeModelAttributes(bool force = false)
        {
            ...
            { 
                Console.WriteLine(e.Message + " | Current Class View: " + GetType().Name + " | Current Type Model: " + modelType?.Name + "\n");
                throw;
            }
        }
        ...
        {
            _methodStorage?.GetList(GameControllerEvents.OnDrag).ForEach(it =>
            {
                it.MethodInfo.Invoke(it.AssociatedObject, new object[] {eventData});
            });
        }
        public void OnEndDrag(PointerEventData eventData)
        {
            _methodStorage?.GetList(GameControllerEvents.OnEndDrag).ForEach(it =>
            {
                it.MethodInfo.Invoke(it.AssociatedObject, new object[] {eventData});
            });
        }
        ...
        {
            _methodStorage?.GetList(GameControllerEvents.OnBeginDrag).ForEach(it =>
            {
                it.MethodInfo.Invoke(it.AssociatedObject, new object[] {eventData});
            });
        }
        ...
        {
            _methodStorage?.GetList(GameControllerEvents.OnBecameInvisible).ForEach(it =>
            {
                it.MethodInfo.Invoke(it.AssociatedObject, null);
            });
        }
